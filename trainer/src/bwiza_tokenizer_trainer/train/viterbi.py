...
def segment_normalized(normalized: str, model: ModelV1) -> SegmentationResult:
    ...
    return SegmentationResult(
        score=result.score,
        ids=tuple(entry.id for entry in result.entries),
        pieces=tuple(entry.piece for entry in result.entries),
    )
